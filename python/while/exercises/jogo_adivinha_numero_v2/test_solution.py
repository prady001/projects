...
def test_acerta_de_primeira(capsys, run_program, mock_input, seed, chute):
    ...
    mensagem = f'Algo deu errado! Era esperado "Acertou", mas foi obtido {obtido}.'

    assert obtido.count('Muito alto') == 0, f"{mensagem}\nNão deveria ter impresso 'Muito alto'.'"
    assert obtido.count('Muito baixo') == 0, f"{mensagem}\nNão deveria ter impresso 'Muito baixo'.'"
    assert obtido.count('Acertou') == 1, f"{mensagem}\nDeveria ter impresso 'Acertou' uma vez.'"
...
def test_algumas_tentativas(capsys, run_program, mock_input, detalhes):
    ...
    assert obtido.count('Acertou') == 1, f"Algo deu errado!\nDeveria ter impresso 'Acertou' uma vez.'"
